...
func (h handler) PriceUpdateHandler(c *fiber.Ctx) (err error) {
	var req = Request{}
	id := c.Params("id")
	...
	result, errors, err := h.service.PriceUpdate(c.Context(), req, id)
	if errors != nil {
		return c.Status(422).JSON(fiber.Map{
			"errors": errors,
		})
...
func (h handler) LimitUpdateHandler(c *fiber.Ctx) (err error) {
	var req = Request{}
	id := c.Params("id")
	...
	result, errors, err := h.service.LimitUpdate(c.Context(), req, id)
	if errors != nil {
		return c.Status(422).JSON(fiber.Map{
			"errors": errors,
		})
